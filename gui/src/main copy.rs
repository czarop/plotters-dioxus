...
fn arcsinh_transform_series(col_data: Column, cofactor: f64) -> PolarsResult<Column> {
    ...
    Ok(transformed_chunked.into_column())
}
    ...
    let transformed_lazy_df = lazy_df.with_columns([
        col(col1_name)
            .map(
                ...
                |_, _| Ok(Field::new("name".into(), DataType::Float64))
            )
            .alias(col1_name),
        col(col2_name)
            .map(
