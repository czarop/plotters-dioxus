use crate::{BufReader, FcsError, File, HashMap, SeekFrom};
...
impl FlowSample {
    ...
    pub fn get_dataframe_columns(&self) -> Vec<String> {
        ...
        let names = ColumnNames { keys_vec };
...
pub fn parse_data(
    ...
    if mode != "L" {
        return Err(FcsError::InvalidData(
            "Data must be in list (L) mode".to_string(),
        ));
    }
    ...
    if capacity == 0 {
        return Err(FcsError::InvalidData(
            "Fcs file may be corrupted. No data found".to_string(),
        ));
    }
    ...
    for i in 1..=n_params {
        events = if byte_order == "1,2,3,4" {
            ...
            return Err(FcsError::InvalidData(
                "Could not determine byte order.".to_string(),
            ));
        };

        let id_1 = metadata.get(&format!("$P{}S", i));
        let id = match id_1 {
            Some(id) => id,
            None => metadata
                .get(&format!("$P{}N", i))
...
pub fn create_dataframe(
    ...
    for (i, column_data) in data.iter().enumerate() {
        ...
        let column = Column::new((&column_titles[i]).into(), column_data);
...
mod tests {
    use super::*;
    use crate::{FcsFile, read_metadata};
    use std::fs::File;
    use std::io::BufReader;
    ...
    fn test_flow_sample_display() {
        ...
        let data = DataFrame::new(vec![
            Column::new("FSC".into(), &[1.0, 2.0, 3.0]),
            Column::new("SSC".into(), &[4.0, 5.0, 6.0]),
        ])
        .unwrap();

        let flow_sample = FlowSample { data, parameters };
...
";
    ...
    fn test_arcsinh_transform() {
        let fcs_file =
            FcsFile::open("./examples/20200624 LEGENDplex_20200808 CMVMRC5 NY3 pDC.813537.fcs")
                .unwrap();
        ...
        assert!(
            flow_sample.arcsinh_transform(5.0, &column_names).is_ok(),
            "{}",
            false
        );

        let fsc: Vec<f64> = flow_sample
            .data
            .column("FSC-H")
            .unwrap()
            .f64()
        ...
        assert_eq!(
            fsc[..=2],
            vec![22.692050373389968, 21.3191345559628, 5.869126409425455]
        );
        assert_eq!(
            ssc[..=2],
            vec![7.3780713106122935, 23.476587479533844, 23.01206351580942]
        );
    }

    #[test]
    fn test_parse_data() {
        let file = File::open("./examples/20200624 LEGENDplex_20200808 CMVMRC5 NY3 pDC.813537.fcs")
            .unwrap();
        let mut reader = BufReader::new(&file);
    ...
    fn test_read_events() {
        ...
        let metadata = read_metadata(&mut reader).expect("Failed to read metadata");
        reader
            .seek(SeekFrom::Start(150))
            .expect("Failed to seek to data start");

        let events = read_events::<LittleEndian>(&mut reader, "F", 3, 1, &metadata)
            .expect("Failed to read events");
        assert_eq!(
            events,
            vec![2.745084202615544e-6, 11018227712.0, 6.37629560262809e-10]
        );
    }
    ...
    fn test_create_dataframe_mismatch() {
        ...
        assert!(
            result.is_err(),
            "DataFrame creation with mismatched columns and data should fail"
        );
    }
}
